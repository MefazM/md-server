...
module NETWORK_SEND_DATA
  ...
  def send_spell_cast(spell_uid, target_area, owner_id)
    message = [SEND_SPELL_CAST_ACTION]
    message << spell_uid
    message << target_area
    message << owner_id
  ...
  def send_battle_sync(units_sync_data_arr, buildings_sync_data_arr)
    message = [SEND_BATTLE_SYNC_ACTION]
  ...
  def send_sync_building_state(uid, level, is_ready = true, finish_time = nil)
    message = [SEND_SYNC_BUILDING_STATE_ACTION]
    message << uid
    message << level
    message << is_ready
  ...
  def send_create_new_battle_on_client(uid, player_units, opponents_main_buildings)
    message = [SEND_CREATE_NEW_BATTLE_ON_CLIENT_ACTION]
    message << uid
